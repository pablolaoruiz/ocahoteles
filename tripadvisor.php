...
class tripadvisor {
	private $config = [
		...
		'v'          => '2.0',
		'url'        => 'https://api.tripadvisor.com/api/partner/',
		'comment'    => true
	];
  ...
  */	
	...
	private function getDetailFormatter() {
  	...
  	public function getHotels($values,$q) {
			$data = $this->getQuery('location_mapper',$values,['q'=>$q,'lang'=>'es']);
			if (!isset($data[0]['location_id'])) return false;
			$this->config['comment'] = false;
			return $this->getQuery('location',$data[0]['location_id'],['lang'=>'es']);		
