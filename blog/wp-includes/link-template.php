...
function get_adjacent_post_link( $format, $link, $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	...
	if ( ! $post ) {
		...
		$string = '<a href="' . get_permalink( $post ) . '" rel="'.$rel.'"><img src="'.get_the_post_thumbnail_url($post, $size = 'relacionado').'" class="img-responsive"><div class="card-img-overlay"><h4 class="card-title">';
		$inlink = str_replace( '%title', $title, $link );
		$inlink = str_replace( '%date', $date, $inlink );
		$inlink = $string . $inlink . '</h4></div></a>';
...
function get_the_post_navigation( $args = array() ) {
	...
	$previous = get_previous_post_link(
		'<div class="col-sm-6 pd-l-5 pd-r-5"><div class="card card-inverse">%link</div></div>',
		$args['prev_text'],
		$args['in_same_term'],
		$args['excluded_terms'],
		$args['taxonomy']
